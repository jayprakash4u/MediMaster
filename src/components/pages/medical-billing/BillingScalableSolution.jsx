const facilities = [
  ...
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-14 h-14">
        <rect x="8" y="12" width="48" height="44" rx="4" stroke="#6366f1" strokeWidth="2.5" fill="none"/>
        ...
        <rect x="16" y="42" width="32" height="10" rx="2" stroke="#6366f1" strokeWidth="2" fill="none"/>
      </svg>
    ),
  },
  {
    ...
    <section className="bg-white py-16 px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="text-gray-500 text-base leading-relaxed max-w-4xl mb-12">
          Bring billing management into your unique clinical workflow with easy
          payment processing, real-time revenue tracking, automatic integration with
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-4">
          {facilities.map((item) => (
            <div
              key={item.label}
              className="flex flex-col items-center gap-4 bg-slate-50 rounded-2xl p-5 hover:bg-emerald-50 transition-colors duration-200 group cursor-pointer"
            >
              <div className="flex items-center justify-center">
                {item.icon}
              </div>
              <p className="text-xs font-semibold text-gray-700 text-center leading-snug group-hover:text-emerald-700 transition-colors">
                {item.label}
              </p>
            </div>
          ))}
