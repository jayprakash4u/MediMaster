...
const targetPoints = [
  "Individuals seeking proactive health management and wellness tracking",
  "Health clinics requiring AI-powered patient analytics and risk scoring",
  "Corporate wellness programs needing employee health monitoring",
  "Insurance providers wanting predictive health data for underwriting",
];
...
export default function MediFitTargetAudience() {
  const colorMap = {
    teal: {
    ...
    <section className="section-shell w-full bg-white font-sans select-none">
      <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8 lg:space-y-12">
        <h2 className="product-section-title text-slate-900">
          Who Is This <span className="text-navy-600">Health Analytics Platform For?</span>
        </h2>
          ...
          <div className="lg:col-span-5 space-y-6">
            {targetPoints.map((text, index) => (
              <div key={index} className="flex items-start gap-3 group">
                ...
                <p className="text-slate-600 font-medium text-lg leading-relaxed">{text}</p>
              </div>
            ))}
          </div>
            ...
            {cards.map((card, index) => {
              ...
              return (
                ...
                >
                  ...
                  <h3
                    className={`font-bold text-sm md:text-base tracking-tight whitespace-normal sm:whitespace-nowrap ${c.heading}`}
                  >
                    {card.title}
                  </h3>
                </div>
