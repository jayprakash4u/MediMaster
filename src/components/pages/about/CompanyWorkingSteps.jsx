...
const workingSteps = [
  {
    ...
    description: "Deep dive into technical requirements, user personas, and data architectures to map clean interface flows.",
    icon: (
  ...
  },
  {
    ...
    description: "Engineering wireframes and humanized layouts built for clarity, high conversion rates, and fast navigation pacing.",
    icon: (
      <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    description: "Writing ultra-performant, optimized front-end components seamlessly integrated with powerful backend frameworks.",
    icon: (
      <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    description: "Strict latency screening, responsive rendering diagnostics, and full functional alignment testing before delivery.",
    icon: (
      <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function CompanyWorkingSteps() {
  ...
  return (
    <section ref={containerRef} className="w-full bg-white py-12 md:py-16 font-sans text-gray-600 antialiased overflow-hidden">
          ...
          <div className="w-full lg:w-1/2 flex justify-center lg:justify-end relative z-10">
            <div className="main-hub-circle relative w-56 h-56 sm:w-64 sm:h-64 rounded-full bg-teal-500/10 flex items-center justify-center p-4 shadow-[0_15px_40px_-10px_rgba(13,148,136,0.2)] border border-teal-500/20">
              <div className="w-full h-full rounded-full bg-white flex flex-col justify-center items-center text-center p-6 shadow-[0_10px_25px_-5px_rgba(0,0,0,0.05)] border border-gray-100">
                <span className="text-[10px] uppercase tracking-[0.25em] text-teal-600 font-black mb-1">Our Process</span>
            ...
            {workingSteps.map((step, idx) => (
              <div key={idx} className="flex items-start relative group">
                ...
                <div className="hidden lg:block absolute left-[36px] w-[140px] pointer-events-none">
                  ...
                  {idx === 4 || idx === workingSteps.length - 1 && (
